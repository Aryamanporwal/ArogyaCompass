...
import { getAssistantsByLabId } from "@/lib/actions/assistant.action";
...
export default function LabDashboard({ params }: PageProps) {
   const [showAllAppointments, setShowAllAppointments] = useState(false);
   const [lab, setLab] = useState<docData | null>(null);
  ...
  useEffect(() => {
    const fetchLab = async () => {
      const doc = await getLabById(params.labId);
      ...
      if (doc && typeof doc === "object") {
        setLab({
          ...
          logoUrl: doc.logoUrl,
          test: Array.isArray(doc.test) ? doc.test : [], // Ensure test is an array
        });
      } else {
        setLab(null);
      }
        ...
        fetchPatientsForAppointments();
          ...
          useEffect(() => {
            const fetchAssistants = async () => {
              if (params.labId) {
                const result = await getAssistantsByLabId(params.labId);
          ...
          useEffect(() => {
           ...
           const handleResetLabPasskey = async () => {
            ...
            const res = await handleResetPasskey(params.labId);
            if (res) {
...
{/* 
        ...
        {selectedNav === "Add Patient" && (
            <>
                {/* Greeting */}
                <div className="mb-6">
                <h1 className="text-xl font-semibold">Hy, Laboratory</h1>
                ...
                <div className="mt-4">
                    {lab && <AssistantRegistration doctorId={params.labId} />}
                </div>
            </>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-8 items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full">
                <div>
                  ...
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{lab?.name}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Email</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white break-all">{lab?.email}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Phone Number</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{lab?.phone}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Address</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{lab?.address || "N/A"}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">City</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{lab?.city || "N/A"}</p>
                </div>
                {/* <div>
                  {/* <p className="text-xs text-gray-500 dark:text-gray-400">Speciality</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{?.speciality || "N/A"}</p> */}
                {/* </div> */}
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">License Number</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{lab?.licenseNumber || "N/A"}</p>
                </div>
                {/* <div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
                ...
                <button
                  onClick={() => {
                    ...
                    const firstTwo = LabName.slice(0,2).toUpperCase();
                    const expectedKey = `LAB${firstTwo}`;
